...
// where h(t) = 1 / sqrt(2 * pi * param(3)) * exp(-(t-param(2))^2 / (2 * param(3)))
// Then:
...
arma::vec Gaussian::h( arma::vec x ) {
    return arma::exp(- .5 * (x - param(2)) % (x - param(2)) / param(3)) / sqrt(2 * arma::datum::pi * param(3));
}

arma::cx_vec Gaussian::H( arma::vec xi ) {
    return arma::exp(- i * param(2) * xi) % arma::exp(- .5 * param(3) * xi % xi);
}
